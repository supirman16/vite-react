...
import { LayoutDashboard, Star, FileText, User, DollarSign, Settings, Users, TestTube2, Ticket, BarChart3, Building2, LogOut, X, ShieldCheck, Megaphone, Award } from 'lucide-react';

export default function Navigation({ onClose }: { onClose?: () => void }) {
    const context = useContext(AppContext);
    if (!context || !context.session) return null; 
    ...
    return (
        <aside className="flex h-full flex-col w-64 bg-white dark:bg-stone-900 shadow-xl border-r border-stone-200 dark:border-stone-800">
            <div className="flex items-center justify-between px-6 h-20 border-b border-stone-200 dark:border-stone-800">
                ...
                <button onClick={onClose} className="md:hidden p-1 text-stone-500 dark:text-stone-400"><X className="h-6 w-6" /></button>
            </div>
            <nav className="flex-1 overflow-y-auto p-4 space-y-1">
                {accessibleNavItems.map(item => (
                    <button key={item.id} onClick={() => handleLinkClick(item.id)} className={`w-full flex items-center px-4 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 ease-in-out group ${ page === item.id ? 'bg-purple-100/50 dark:bg-cyan-400/10 text-purple-600 dark:text-cyan-300 nav-item-active' : 'text-stone-600 hover:bg-stone-100 dark:text-stone-300 dark:hover:bg-stone-800 hover:translate-x-1'}`}>
            ...
            <div className="p-4 border-t border-stone-200 dark:border-stone-800">
                <div className="flex items-center gap-3 mb-4 p-2 rounded-lg bg-stone-100 dark:bg-stone-800/50">
                    <div className="h-10 w-10 rounded-full unity-gradient-bg text-white flex items-center justify-center font-bold text-lg flex-shrink-0">{userInitial}</div>
                    <div className="overflow-hidden">
                        <p className="text-sm font-semibold text-stone-800 dark:text-white truncate">{session.user.email}</p>
                        <div className="flex items-center"><ShieldCheck className="h-4 w-4 text-green-500 mr-1.5" /><p className="text-xs font-medium text-stone-500 dark:text-stone-400 capitalize">{userRole}</p></div>
                    </div>
                </div>
                <button onClick={logout} className="w-full flex items-center justify-center px-4 py-2.5 text-sm font-semibold rounded-lg text-red-600 hover:bg-red-100 dark:text-red-400 dark:hover:bg-red-500/10 transition-colors"><LogOut className="h-5 w-5 mr-2" /><span>Keluar</span></button>
            </div>
        </aside>
    );
}
